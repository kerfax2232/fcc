...
function largestOfFour(arr) {
  // You can do this!
  return arr.map(function(group) { //combines subarrays into one array
   return group.reduce(function(prev, current) { // reduces each subarray to one digit subarray
     return (current > prev) ? current : prev; // checks if current is > previous, replaces prev with current number if true
   }); 
  });
}
...
Advanced answer (still need to research to understand, but looks better)
